...
/*void FeatureManager::triangulateLinepoint(Vector3d Ps[], Vector3d tic[], Matrix3d ric[])
{
    for (auto &it_per_id : line_feature)
    {
        ...
        auto triangulateEndpoint = [&](Eigen::MatrixXd &svd_A, auto &frame_point, double &estimated_depth) {
            int svd_idx = 0;
            Eigen::Matrix<double, 3, 4> P0;
            Eigen::Vector3d t0 = Ps[imu_i] + Rs[imu_i] * tic[0];
            Eigen::Matrix3d R0 = Rs[imu_i] * ric[0];
        ...
        triangulateEndpoint(svd_A_start, it_per_id.feature_per_frame[0].start_point, it_per_id.estimated_depth_start);

        // 终点三角化
        Eigen::MatrixXd svd_A_end(2 * it_per_id.feature_per_frame.size(), 4);
        triangulateEndpoint(svd_A_end, it_per_id.feature_per_frame[0].end_point, it_per_id.estimated_depth_end);
    }
}*/
